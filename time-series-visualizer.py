...
def draw_bar_plot():
  ...
  f = sns.catplot(x="year", y="value", kind="bar", hue="month", hue_order=months, data=df_bar, ci=None, legend=None)
  fig, ax = f.fig, f.ax
  ax.set_xlabel('Years')
...
def draw_box_plot():
  ...
  f = sns.PairGrid(df_box, x_vars=['year', 'month'], y_vars=['value'])
  f.map(sns.boxplot)
  fig = f.fig
